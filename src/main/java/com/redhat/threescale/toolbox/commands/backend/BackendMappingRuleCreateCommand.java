...
public class BackendMappingRuleCreateCommand implements Runnable {
    ...
    private static final Logger LOG = Logger.getLogger(BackendMappingRuleCreateCommand.class);
    ...
    public void run() {
        try {
            accountManagementService.createBackendMappingRule(backendId, accessToken, httpMethod, pattern, backendId, metricId, position, last);
        } catch (Exception e) {
